...
function PlaceRow({ place, rank }: { place: RankedPlace; rank: number }) {
  ...
  return (
    ...
    >
      {/* Rank */}
      <div className={`w-8 text-center shrink-0 ${rank <= 3 ? 'text-xl' : 'text-sm font-bold text-gray-400'}`}>
        {medal(rank)}
      </div>
      ...
      <div className="shrink-0 text-right">
        ...
        <p className="text-[10px] text-gray-400">{place.review_count} rev</p>
      </div>
    </button>
  )
}

function ReviewerRow({ reviewer, rank, currentUserId }: {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-400">
          {reviewer.borough ?? 'NYC'} · {reviewer.total_likes > 0 ? `❤️ ${reviewer.total_likes}` : 'No likes yet'}
        </p>
      </div>
    ...
    <div className="pb-6">
      ...
      <div className="px-4 pt-5 pb-0 bg-white sticky top-0 z-10 border-b border-gray-100">
        ...
        <div className="flex gap-0">
          {([['pizza', 'Best Pizza'], ['reviewers', 'Best Reviewers']] as const).map(([key, label]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
